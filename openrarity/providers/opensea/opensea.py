...
class OpenseaApi:
        ...
        return {
            t["token_identifier"]["token_id"]: {
                "attributes": [
                    ...
                    for name, value in t["metadata_dict"].items()
                ]
            }
            for t in data
        }
    ...
    @classmethod
    def fetch_opensea_collection_data(cls, slug: str) -> dict:
        ...
        """
        response = httpx.get(cls.OS_COLLECTION_URL.format(slug=slug))
    ...
    def fetch_opensea_assets_data(
        cls, slug: str, token_ids: list[int], limit=30
    ) -> list[dict]:
        ...
        response = httpx.get(cls.OS_ASSETS_URL, headers=cls.HEADERS, params=querystring)
...
#     batch_size: int
#         batch size for the opensea API httpx
#         maximum value is 30
