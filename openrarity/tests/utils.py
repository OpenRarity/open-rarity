...
from openrarity.models.token_metadata import TokenMetadata, StringAttributeValue, AttributeName
...
def generate_uniform_attributes_count(
    ...
    if token_total_supply%values_per_attribute>0:
        raise Exception("token_total_supply must be divisible by values_per_attribute for uniform count metadata")

    return {str(i):{str(j):token_total_supply//values_per_attribute for j in range(values_per_attribute)} for i in range(attribute_count)}
...
def generate_uniform_rarity_collection(
    ...
    collection_attributes_count = generate_uniform_attributes_count(attribute_count, values_per_attribute, token_total_supply)
...
def generate_onerare_attributes_count(
    ...
    """generate trait counts with a single token with one rare attribute; otherwise uniformly distributed"""

    if (token_total_supply-1)%(values_per_attribute-1)>0:
        raise Exception("token_total_supply-1 must be divisible by values_per_attribute-1 for onerare count metadata")

    collection_attributes_count = {str(i):{str(j):token_total_supply//values_per_attribute for j in range(values_per_attribute)} for i in range(attribute_count-1)}
    
    for j in range(values_per_attribute):
        if j<values_per_attribute-1:
            trait_count=(token_total_supply-1)//(values_per_attribute-1)
        else:
            trait_count=1
        collection_attributes_count[str(attribute_count-1)][str(j)] = trait_count
...
def generate_onerare_rarity_collection(
    ...
    """generate a Collection with a single token with one rare attribute; otherwise uniformly distributed attributes"""
